...
Usaremos o pacote `dplyr` para manipular efetivamente e calcular condicionalmente as estatísticas sumárias sobre subconjuntos de um conjunto de dados “grande” contendo muitas observações.


### Os dados

Usaremos um conjunto de dados de expressão gênica de levedura. Esta é uma versão "limpa" dos dados de expressão gênica de [Brauer et al. Coordination of growth rate, cell cycle, stress response, and metabolic activity in yeast. (2008) Mol Biol Cell 19: 352-367](https://www.ncbi.nlm.nih.gov/pubmed/17959824). 

Esses dados são de um microarranjo de expressão gênica, e neste artigo os autores estão examinando a relação entre taxa de crescimento e expressão gênica em culturas de leveduras limitadas por um dos seis nutrientes diferentes (glicose, leucina, amônio, sulfato, fosfato e uracila). Se você propiciar à levedura um meio rico em nutrientes, exceto restringir o fornecimento de um único nutriente, poderá controlar a taxa de crescimento a qualquer taxa que escolher. Por fome de levedura de nutrientes específicos, você pode encontrar genes que:
...
1. Selecione os dados onde o processo biológico da ontologia genética (a variável `bp`) é a “biossíntese de leucina” e o nutriente limitante foi a Leucina. Armazene sua resposta em um novo dataframe `leudat`, que deve ser um dataframe de 24 x 7. Quantos genes resultantes e quantas taxas de crescimento temos nessa seleção?
...
8. Mostre os valores limitantes de nutrientes e expressão para o gene ADH2 quando a taxa de crescimento é restrita a 0.05. **Dica:** 2 pipes: `filter()` e `select()`.
...
10. Quando a taxa de crescimento é restrita a 0.05, qual é o nível médio de expressão em todos os genes no processo biológico de “resposta ao estresse”, separadamente para cada nutriente limitante? E quanto aos genes no processo biológico da "biossíntese de proteínas"? **Dica:** 3 pipes: `filter()`, `group_by()`, `summarize()`.